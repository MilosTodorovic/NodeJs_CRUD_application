exports.InstitutionModel = function (dbcon) {
    return {
        ...
        editInstitutionById : function(institutionType, institutionName, stateId, ownershipType, id) {
            return new Promise((resolve, reject) => {
                let query = 'UPDATE HIGH_EDUCATION_INSTITUTION SET VU_IDENTIFIKATOR = ?, TIP_UST = ?, VU_NAZIV = ?, DR_IDENTIFIKATOR = ?, VV_OZNAKA = ? WHERE VU_IDENTIFIKATOR LIKE ?;';
                
                dbcon.query(query, [id, institutionType, institutionName, stateId, ownershipType, id], (err, data) => {
                    if ( !err ) {
                        resolve(data);
                    } else {
                        reject(err);
